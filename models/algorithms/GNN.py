...
class GNN(torch.nn.Module):
    ...
    def fit(self, epochs=300, trajectory=None):
        """`trajectory` is an optional study.core.trajectory.ValidationHistory.

        When None -- the default and every pre-existing caller -- this method is
        unchanged. When supplied, the validation logits already computed below
        are recorded; no extra forward pass is added, so no RNG is consumed and
        training cannot diverge from an uninstrumented run. That is the property
        the instrumentation-invariance test checks.
        """
        best_loss = 100
        self.best_epoch = -1
        for epoch in range(epochs + 1):
            ...
            if trajectory is not None:
                # the same tensor the selector below sees, detached; the
                # forward pass was going to happen anyway
                trajectory.add(epoch,
                               c_val[self.idx_val].squeeze().detach().cpu().numpy(),
                               stage="classifier")
            ...
            if (val_loss) < best_loss:
                ...
                best_loss = val_loss
                self.best_epoch = epoch        # recorded so a replayed selector
    ...
    def predict_sens_group(self, output, idx_test):
        ...
        result=[]
        for sens in [0,1]:
            sens_mask = (self.sens[idx_test].cpu() == sens)
            F1 = f1_score(self.labels[idx_test].cpu().numpy()[sens_mask], pred[sens_mask], average="micro")
            ACC = accuracy_score(self.labels[idx_test].cpu().numpy()[sens_mask], pred[sens_mask])
            AUCROC = roc_auc_score(self.labels[idx_test].cpu().numpy()[sens_mask], pred[sens_mask])
