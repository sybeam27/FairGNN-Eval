...
class FairWalk():
    ...
    def fit(self, adj, labels, idx_train, sens, device="cuda", dimensions: int = 64, walk_length: int = 20, num_walks: int = 5, p: float = 1,
        ...
        # adj.to_dense() 대신 sparse에서 직접 변환
        ...
        self.graph = nx.from_scipy_sparse_array(scipy_adj)
...
#
        ...
        self.idx_train = idx_train  # Weakness 3 rebuttal metrics: leakage probe train split
...
#
    ...
    def predict_extra_fairness(self, idx_test):
        ...
        IMPORTANT DEVIATION from the other patched baselines: FairWalk's
        fit() never receives raw node features (data.x) at all -- it is
        purely walk/structure-based (see fit() signature: adj, labels,
        idx_train, sens -- no feats). So individual_fairness_consistency
        is computed here in the LEARNED EMBEDDING space (self.embs)
        rather than the raw feature space used for every other baseline.

        This is a defensible choice (Dwork et al.'s individual fairness
        definition does not mandate a specific similarity metric space),
        but it means FairWalk's ifair_consistency number is not computed
        on the identical basis as e.g. CrossWalk's or FairGNN's. Flag
        this explicitly if reporting these side by side in the rebuttal.
        To make it fully comparable, fit() would need a `feats` parameter
        threaded through from train_baselines.py's FairWalk branch.
        """
        if not hasattr(self, "embs"):
            raise RuntimeError("Call fit() before predict_extra_fairness().")

        ifair = individual_fairness_consistency(
            self._all_node_probs(idx_test), self.embs, idx_test, k=5)
    ...
    def _all_node_probs(self, idx_test):
        """
        classify() only exposes hard predict() output. Refits the same
        LabelPropagation classifier used in classify() (same embs, same
        idx_test-as-unlabeled split) to obtain soft probabilities, without
