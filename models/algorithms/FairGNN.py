...
class FairGNN(nn.Module):
    ...
    def fit(self, g, features, labels, idx_train, idx_val, idx_test, sens, idx_sens_train,
            device='cuda', trajectory=None):
        ...
        for epoch in tqdm(range(args.epochs)):
            ...
            if trajectory is not None:
                # the validation logits this epoch already produced; no extra
                # forward pass, so no RNG is consumed and training is unchanged
                trajectory.add(epoch,
                               output[idx_val].squeeze().detach().cpu().numpy(),
                               stage="classifier")
