...
def _sn(module, enabled: bool = True):
    """spectral_norm(module), or the bare module when disabled.

    NIFTY claims fairness in two places: the counterfactual objective, and a
    Lipschitz constraint enforced by layer-wise spectral normalisation (UAI'21
    ablates the two separately). This study measured only the first, by setting
    sim_coeff = 0, and described it as "NIFTY off" -- which it is not, because
    every projector, predictor and classifier head stays spectrally normalised.

    `enabled=True` is the default and returns exactly what the original call
    returned, so no existing path changes.
    """
    return spectral_norm(module) if enabled else module


class Classifier(nn.Module):
    def __init__(self, ft_in, nb_classes, use_spectral_norm: bool = True):
        super(Classifier, self).__init__()

        # Classifier projector
        self.fc1 = _sn(nn.Linear(ft_in, nb_classes), use_spectral_norm)
...
class NIFTY(torch.nn.Module):
    def __init__(self, adj, features, labels, idx_train, idx_val, idx_test, sens, sens_idx, num_hidden=16, num_proj_hidden=16, lr=0.001, weight_decay=1e-5, drop_edge_rate_1=0.1, drop_edge_rate_2=0.1, drop_feature_rate_1=0.1, drop_feature_rate_2=0.1, encoder="gcn", sim_coeff=0.5, nclass=1, device="cuda", use_spectral_norm=True):
        ...
        self.fc1 = nn.Sequential(
            _sn(nn.Linear(num_hidden, num_proj_hidden), use_spectral_norm),
            nn.BatchNorm1d(num_proj_hidden),
            nn.ReLU(inplace=True)
        )
        self.fc2 = nn.Sequential(
            _sn(nn.Linear(num_proj_hidden, num_hidden), use_spectral_norm),
            nn.BatchNorm1d(num_hidden)
        )

        # Prediction
        self.fc3 = nn.Sequential(
            _sn(nn.Linear(num_hidden, num_hidden), use_spectral_norm),
            nn.BatchNorm1d(num_hidden),
            nn.ReLU(inplace=True)
        )
        self.fc4 = _sn(nn.Linear(num_hidden, num_hidden), use_spectral_norm)
    ...
    def fit_GNN(self, epochs=300):
        ...
        for epoch in range(epochs + 1):
            ...
            if (val_loss) < best_loss:
                ...
                best_loss = val_loss
                self.best_state_gnn = copy.deepcopy(self.state_dict())




    def fit(self, epochs=300, trajectory=None):
        ...
        for epoch in range(epochs + 1):
            ...
            if trajectory is not None:
                # the validation logits already computed above, detached.
                # val_c_loss and val_s_loss go in `extra` because this method's
                # selector is argmin(val_c_loss + val_s_loss) and neither term
                # is recoverable from the classifier score alone.
                trajectory.add(epoch,
                               output[self.idx_val].squeeze().detach().cpu().numpy(),
                               stage="classifier",
                               extra={"val_c_loss": float(val_c_loss),
                                      "val_s_loss": float(val_s_loss)})
    ...
    def predict_GNN(self):
        ...
        self.load_state_dict(self.best_state_gnn)
        self.eval()
    ...
    def predict(self):
        ...
        self.load_state_dict(self.best_state_ssf)
        self.eval()
        emb = self.forward(self.features.to(self.device), self.edge_index.to(self.device))
        output = self.forwarding_predict(emb)
    ...
    def predict_extra_fairness(self, idx_test):
        """
        Weakness 3 rebuttal metrics. NIFTY's predict() already reloads the
        best-epoch weights (self.best_state_ssf) and recomputes emb/output
        from scratch each call -- so this needs no re-run, just a fresh
        forward pass the same way predict() does.
        """
        self.load_state_dict(self.best_state_ssf)
        self.eval()
        emb = self.forward(self.features.to(self.device), self.edge_index.to(self.device))
        ...
        ifair = individual_fairness_consistency(probs, self.features, idx_test, k=5)
        leak = sensitive_leakage_probe(emb, self.sens, self.idx_train, idx_test)
        return {**ifair, **leak}
