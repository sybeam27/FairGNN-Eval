...
class fair_edit_trainer():
    ...
    def add_drop_edge_random(self, add_prob=0.001, del_prob=0.01):
        ...
        # Generate scores
        scores = torch.Tensor(np.random.uniform(0, 1, (N, N))).to(self.device)
    ...
    def perturb_graph(self, deleted_edges, add_edges):
        ...
        for edge in deleted_edges.T:
            vals = (self.edge_index == torch.tensor([[edge[0]], [edge[1]]]).to(self.device))
    ...
    def fair_graph_edit(self):
        ...
        # we want to add edge since better
            if best_add_score < best_delete_score:
                ...
                best_add_comp = torch.tensor([[best_add[1]], [best_add[0]]]).to(self.device)
        ...
        val_del = (self.edge_index == torch.tensor([[best_delete[1]], [best_delete[0]]]).to(self.device))
        sum_del = torch.sum(val_del, dim=0).to(self.device)
        ...
        best_delete_comp = torch.tensor([[best_delete[1]], [best_delete[0]]]).to(self.device)
        val_del_comp = (self.edge_index == torch.tensor([[best_delete_comp[1]], [best_delete_comp[0]]]).to(self.device))
        sum_del_comp = torch.sum(val_del_comp, dim=0).to(self.device)
    ...
    def train(self, epochs=200):
        ...
        for epoch in tqdm(range(epochs)):
            ...
            if loss_val < best_loss:
                ...
                # best-epoch point (`output` above is already the full-node
                # forward pass). Model weights are NOT restored to this
                # checkpoint after train() returns (see commented-out
                # torch.save above), so this snapshot -- and a re-run after
                # patching -- is required to get correct values.
                self.temp_probs = torch.sigmoid(output.squeeze()).detach()
                with torch.no_grad():
                    if self.model_name == 'gcn':
                        h = self.model.body1(self.features, self.edge_index)
                        h = self.model.body2(h, self.edge_index)
                    elif self.model_name == 'sage':
                        h = self.model.conv1(self.features, self.edge_index)
                        h = self.model.transition(h)
                        h = self.model.conv2(h, self.edge_index)
                    else:
                        h = None  # APPNP: intermediate layers interleave
                                  # dropout with the final projection, no
                                  # clean embedding to extract externally
                self.temp_embeddings = h.detach() if h is not None else None
...
class FairEdit():
    def fit(self, adj, features, labels, idx_train, idx_val, idx_test, sens, sens_idx, model_name="gcn", epochs=100, lr=1e-3, weight_decay=5e-4, hidden=16, dropout=0.5, edit_num=10, device="cuda"):
        ...
        scipy_adj = sp.coo_matrix((values, (indices[0], indices[1])), shape=adj.shape)
        edge_index = convert.from_scipy_sparse_matrix(scipy_adj)[0]
    ...
    def predict_extra_fairness(self, idx_test):
        """
        Weakness 3 rebuttal metrics. NOTE: FairEdit's actual training runs
        inside predict() (self.trainer.train(...)), not fit() -- so this
        must be called AFTER predict(), by which point self.trainer has
        the best-epoch snapshot set.
        """
        tr = self.trainer
        if not hasattr(tr, "temp_probs"):
            raise RuntimeError(
                "No snapshot found. Call predict() first (training happens "
                "there for this model), then call predict_extra_fairness()."
            )
        ifair = individual_fairness_consistency(tr.temp_probs, tr.features, idx_test, k=5)
        result = dict(ifair)
        if tr.temp_embeddings is not None:
            result.update(sensitive_leakage_probe(
                tr.temp_embeddings, tr.sens, tr.train_idx, idx_test))
        return result