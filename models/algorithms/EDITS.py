...
def binarize(A_debiased, adj_ori, threshold_proportion):
    ...
    the_con1 = (A_debiased - adj_ori).toarray()
...
class EDITS(nn.Module):
    ...
    def forward(self, A, X):
        ...
        agg_con = self.propagation_cat_new_filter(X_de, adj_new, layer_threshold=self.layer_threshold)  # A_de or A
        D_pre = self.fc(agg_con)
        D_pre = self.dropout(D_pre)
        return adj_new, X_de, D_pre, D_pre, agg_con

    def optimize(self, adj, features, idx_train, sens, epoch, lr, device="cuda", alpha=5e-2, beta=0.1):
        """
        ...
        if epoch == 0:
            self.adj_renew.fit(adj, self.lr, device=device)
    ...
    def fit(self, adj, features, sens,  idx_train, idx_val, epochs=100, normalize=True, lr=0.003, k=-1, device='cuda', half=True, truncation=4):
        ...
        if half:
            ...
            features_preserve = features_preserve.cuda()
        ...
        for epoch in tqdm(range(epochs)):
            if epoch > 400:
                lr = 0.001
            self.train()
            self.optimize(adj, features, idx_train, sens, epoch, lr, device=device)
    ...
    def predict(self, adj_ori, labels, sens, idx_train, idx_val, idx_test, epochs=100, lr=0.003, nhid=50, dropout=0.2, weight_decay=1e-7, model='GCN', device='cuda', threshold_proportion = 0.015):
        ...
        """
        indices = adj_ori.coalesce().indices().cpu().numpy()
        values = adj_ori.coalesce().values().cpu().numpy()
        adj_ori = sp.coo_matrix((values, (indices[0], indices[1])), shape=adj_ori.shape)
        ...
        edge_index = convert.from_scipy_sparse_matrix(A_debiased)[0].to(device)
        ...
        model.to(device)
        X_debiased = X_debiased.to(device)
        ...
        def train(epoch, pa, eq, test_f1, val_loss, test_auc):
            ...
            output = model(x=X_debiased, edge_index=torch.LongTensor(edge_index.cpu()).to(device))
            ...
            if loss_val < val_loss:
                ...
                ACC, AUCROC, F1, ACC_sens0, AUCROC_sens0, F1_sens0, ACC_sens1, AUCROC_sens1, F1_sens1, SP, EO = test(idx_test.cpu())
                val_ACC, val_AUCROC, val_F1, val_ACC_sens0, val_AUCROC_sens0, val_F1_sens0, val_ACC_sens1, val_AUCROC_sens1, val_F1_sens1, val_SP, val_EO = test(idx_val.cpu())

                # ── Weakness 3 rebuttal metrics: snapshot at this same
                # best-epoch point. `self` is reachable here via closure
                # (train() is nested inside predict()). Model weights are
                # not restored to this checkpoint afterward, so a re-run
                # after patching is required.
                self.temp_probs = torch.sigmoid(output.squeeze()).detach()
                with torch.no_grad():
                    h_full = model.body(X_debiased, edge_index)
                self.temp_embeddings = h_full.detach()
                self.temp_idx_train  = idx_train
        ...
        for epoch in tqdm(range(epochs)):
            ...
            val_ACC, val_AUCROC, val_F1, val_ACC_sens0, val_AUCROC_sens0, val_F1_sens0, val_ACC_sens1, val_AUCROC_sens1, val_F1_sens1, val_SP, val_EO  = train(epoch, pa, eq, test_f1, val_loss, test_auc)
        ...
        self.val_ACC, self.val_AUCROC, self.val_F1, self.val_ACC_sens0, self.val_AUCROC_sens0, self.val_F1_sens0, self.val_ACC_sens1, \
        self.val_AUCROC_sens1, self.val_F1_sens1, self.val_SP, self.val_EO = val_ACC, val_AUCROC, val_F1, val_ACC_sens0, \
                                                                             val_AUCROC_sens0, val_F1_sens0, val_ACC_sens1, val_AUCROC_sens1, val_F1_sens1, val_SP, val_EO

        return ACC, AUCROC, F1, ACC_sens0, AUCROC_sens0, F1_sens0, ACC_sens1, AUCROC_sens1, F1_sens1, SP, EO

    def predict_val(self):
        return self.val_ACC, self.val_AUCROC, self.val_F1, self.val_ACC_sens0, self.val_AUCROC_sens0, self.val_F1_sens0, self.val_ACC_sens1, self.val_AUCROC_sens1, self.val_F1_sens1, self.val_SP, self.val_EO

    def predict_extra_fairness(self, idx_test):
        """
        Weakness 3 rebuttal metrics. Training happens inside predict()
        (not fit(), which only debiases features/adjacency) -- so this
        must be called AFTER predict(), by which point the best-epoch
        snapshot (self.temp_probs / self.temp_embeddings) is set.
        """
        if not hasattr(self, "temp_probs"):
            raise RuntimeError(
                "No snapshot found. Call predict(...) first, then call "
                "predict_extra_fairness()."
            )
        ifair = individual_fairness_consistency(
            self.temp_probs, self.X_debiased, idx_test, k=5)
        leak = sensitive_leakage_probe(
            self.temp_embeddings, self.sens, self.temp_idx_train, idx_test)
        return {**ifair, **leak}
...
class Adj_renew(nn.Module):
    ...
    def fit(self, adj, lr, device="cuda"):
