...
class CrossWalk():
    ...
    def process(self, adj_matrix):
        ...
        scipy_adj_cw = sp.coo_matrix((values, (indices[0], indices[1])), shape=adj_matrix.shape)
        G = from_numpy(scipy_adj_cw, undirected=True)
...
#
    ...
    def fit(self, adj_matrix, feats, labels, idx_train, sens, device="cuda", number_walks=5, representation_size=64, seed=0, walk_length=20, window_size=5, workers=5, pmodified=1.0, deep_walk=False):
        ...
        indices = adj_matrix.coalesce().indices().cpu().numpy()
        values = adj_matrix.coalesce().values().cpu().numpy()
        scipy_adj = sp.coo_matrix((values, (indices[0], indices[1])), shape=adj_matrix.shape)
        edge_index = from_scipy_sparse_matrix(scipy_adj)[0]
...
#
    ...
    def predict_extra_fairness(self, idx_test):
        """
        Weakness 3 rebuttal metrics.

        Unlike the GNN baselines, CrossWalk's embeddings (self.embs, from
        Node2Vec) persist unchanged after fit() -- there is no "best
        checkpoint vs. final weights" mismatch here, so no re-run is
        strictly required as long as fit() has already been called with
        this patch present (self.embs / self.idx_train are set inside
        fit(), which does not change behaviorally with this patch).

        individual_fairness_consistency uses self.feats (the original raw
        node features, stored in fit()) for the kNN similarity space, for
        comparability with the other (GNN-based) baselines' use of data.x.
        """
        if not hasattr(self, "embs"):
            raise RuntimeError("Call fit() before predict_extra_fairness().")

        raw_feats = self.feats.detach().cpu().numpy() if torch.is_tensor(self.feats) else self.feats
        ifair = individual_fairness_consistency(
            self._all_node_probs(idx_test), raw_feats, idx_test, k=5)
        leak = sensitive_leakage_probe(
            self.embs, self.sens, self.idx_train, idx_test)
        return {**ifair, **leak}

    def _all_node_probs(self, idx_test):
        """
        CrossWalk's classify() only exposes hard predict() output, not
        probabilities. Refits the same LabelPropagation classifier used in
        classify() (same embs, same idx_test-as-unlabeled split) to obtain
        soft probabilities for individual_fairness_consistency, without
        touching classify() itself (so the officially reported metrics from
        predict() are unaffected by this addition).
        """
        from sklearn.semi_supervised import LabelPropagation
        from sklearn.metrics import pairwise_distances

        idx_test_np = idx_test.cpu().numpy() if torch.is_tensor(idx_test) else np.asarray(idx_test)
        X = self.embs
        y_train = self.labels.copy() if hasattr(self.labels, "copy") else np.asarray(self.labels).copy()
        y_train[idx_test_np] = -1

        g = np.mean(pairwise_distances(X))
        clf = LabelPropagation(gamma=g).fit(X, y_train)
        proba_all = clf.predict_proba(X)  # [N, 2]
        return proba_all[:, 1]
