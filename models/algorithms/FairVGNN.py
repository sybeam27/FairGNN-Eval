...
def evaluate_ged3(x, classifier, discriminator, generator, encoder, data, args,
    ...
    Exposing them matters more here than elsewhere: this function calls
    F.gumbel_softmax(..., hard=True), which is stochastic and consumes RNG
    even under torch.no_grad(). A logger that re-invoked it to obtain the
    scores would advance the generator and change training. The only safe
    hook is the output of the call the loop already makes.
    """
    classifier.eval()
    generator.eval()
    encoder.eval()
...
def get_dataset(dataname, top_k,adj, features, labels, idx_train, idx_val, idx_test, sens, sens_idx):
    ...
    edge_index, _ = from_scipy_sparse_matrix(sp.coo_matrix((adj.coalesce().values().cpu().numpy(), (adj.coalesce().indices()[0].cpu().numpy(), adj.coalesce().indices()[1].cpu().numpy())), shape=adj.shape))
...
class FairVGNN():
    ...
    def inference_modules(self):
        """Modules a saved InferenceState restores into; see run()."""
        return dict(self._inf_modules)

    def fit(self, adj, feats, labels, idx_train, idx_val, idx_test, sens, sens_idx, device='cuda', runs=1, epochs=200, d_epochs=5, g_epochs=5,
            c_epochs=5, g_lr=0.001, g_wd=0, d_lr=0.001, d_wd=0, c_lr=0.001, c_wd=0, e_lr=0.001, e_wd=0, early_stopping=0,
            prop='spmm', dropout=0.5, hidden=16, seed=1, encoder='GCN', K=10, top_k=10, clip_e=1, f_mask='yes',weight_clip='yes',ratio=1, alpha=1, trajectory=None):
        """`trajectory` optional; None leaves every existing caller unchanged."""
        ...
        self.args=args
        print('running')
        self.acc, self.f1, self.auc_roc, self.parity, self.equality, self.parity_val, self.equality_val, self.test_acc_sens0, self.test_acc_sens1, self.test_auc_roc_sens0, self.test_auc_roc_sens1, self.test_f1_sens0, self.test_f1_sens1 = self.run(data, args, trajectory=trajectory)
    ...
    def predict(self):
        ...
        return float(np.mean(acc)), float(np.mean(auc_roc)), float(np.mean(f1)), test_acc_sens0, test_auc_roc_sens0, test_f1_sens0, test_acc_sens1, test_auc_roc_sens1, test_f1_sens1, float(np.mean(parity)), float(np.mean(equality))
    ...
    def predict_extra_fairness(self, idx_test):
        """
        Weakness 3 rebuttal metrics, from the best-epoch snapshot captured
        inside run(). Requires fit() to have been re-run with this patch
        present (a completed pre-patch run has no temp_* attributes).

        NOTE: like FairWalk, this class never stores raw node features
        (data.x) on self -- `data` only exists as a local variable inside
        run(). individual_fairness_consistency therefore uses the learned
        embedding space here rather than raw features, unlike most other
        patched baselines; flag this if comparing ifair_consistency across
        models side by side in the rebuttal.
        """
        ...
        ifair = individual_fairness_consistency(
            self.temp_probs, self.temp_embeddings, idx_test, k=5)
        leak = sensitive_leakage_probe(
            self.temp_embeddings, self.temp_sens, self.temp_idx_train, idx_test)
        return {**ifair, **leak}