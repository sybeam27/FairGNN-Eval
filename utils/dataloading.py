...
def load_data(dataset: str, feature_normalize: bool = True,
              split_seed: int = 20):
    """
    split_seed 는 get_dataset 으로 그대로 전달된다. 이 인자가 없던 동안
    이 어댑터를 거치는 여덟 개 모델(FairGNN, NIFTY, EDITS, FairEdit,
    FairVGNN, FairWalk, CrossWalk, GNN)은 --split_seed 를 조용히 무시하고
    언제나 split 20 을 썼다. FairGB 와 FairGT 만 get_dataset 을 직접 호출해
    그 인자를 존중했다 -- train_baselines.py 의 --split_seed 도움말이 그
    한계를 명시하고 있었다. 분할을 바꿔가며 재평가하려면 반드시 필요하다.
    ...
    반환값:
        ...
        sens_idx    : int
    """
    data, sens_idx, x_min, x_max = get_dataset(
        dataset, feature_normalize=feature_normalize, split_seed=split_seed)
