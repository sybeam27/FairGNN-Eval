...
# Imported on demand rather than at module load. Importing all ten pulls in
# torch_scatter/torch_sparse (FairVGNN), gensim (FairWalk/CrossWalk) and
# deeprobust + aif360 (EDITS/FairEdit), so running FairGNN required every
# dependency of every other method. That is not merely inconvenient: installing
# deeprobust upgraded torch from 2.6.0+cu124 to 2.14.0+cu130, which broke the
# compiled torch_scatter extension and would have silently changed the
# environment every result in harness/ was produced under. Resolving a model name
# only when it is asked for keeps each method's dependencies to itself.
_MODEL_IMPORTS = {
    "GNN":       ("algorithms.GNN", "GNN"),
    "FairGNN":   ("algorithms.FairGNN", "FairGNN"),
    "FairVGNN":  ("algorithms.FairVGNN", "FairVGNN"),
    "FairWalk":  ("algorithms.FairWalk", "FairWalk"),
    "CrossWalk": ("algorithms.CrossWalk", "CrossWalk"),
    "EDITS":     ("algorithms.EDITS", "EDITS"),
    "FairEdit":  ("algorithms.FairEdit", "FairEdit"),
    "NIFTY":     ("algorithms.NIFTY", "NIFTY"),
    "FairGB":    ("algorithms.FairGB_alg", "FairGB"),
    "FairGT":    ("algorithms.FairGT_alg", "FairGT"),
}


def _model_class(name: str):
    import importlib
    mod, cls = _MODEL_IMPORTS[name]
    return getattr(importlib.import_module(mod), cls)


def __getattr__(name):
    """PEP 562 lazy attribute, for importers of this module. Call sites *inside*
    this file use `_model_class(...)` directly, because a bare name there is a
    LOAD_GLOBAL and never reaches here."""
    if name in _MODEL_IMPORTS:
        return _model_class(name)
...
def _fgnn_off(component: str, args) -> bool:
    """Is this named component switched off? `--fairness_off` means all of them."""
    if getattr(args, "fairness_off", False):
        return True
    return component in set(getattr(args, "components_off", []) or [])


def _fn(default, args):
    """feature_normalize, with an audit-wide override.

    Each branch below carries the normalisation its original implementation
    used -- GNN and NIFTY none, FairGNN only on NBA and German, FairGB/FairGT
    whatever `get_dataset` defaults to. The paper states that "all methods share
    the same datasets, splits, seeds, learning rate and weight decay"; the
    preprocessing is not in that list and is not in fact shared, and a direct
    test put the effect at +0.074 AUC on Income and -0.050 on Credit -- the same
    order as the differences between methods, and not even the same sign.

    `--force_feature_normalize` runs the whole comparison at one setting so the
    two arms can be differenced. It defaults to None, which leaves every branch
    exactly as submitted.
    """
    f = getattr(args, "force_feature_normalize", None)
    return default if f is None else bool(int(f))
...
def _maybe_add_extra_fairness(model, result: dict, **kwargs) -> dict:
    """
    Duck-typed hook for Weakness 3 (ydje) / Point 3 (Kips) rebuttal metrics.

    If `model` exposes a `predict_extra_fairness(...)` method (added via
    a per-algorithm patch, e.g. FairGNN.py), its output (ifair_consistency,
    leakage_probe_auc, acc_gap, ...) is merged into `result`.

    Models that have NOT been patched yet are silently skipped (no error,
    no missing columns injected) -- so baselines can be migrated to the
    new metrics one at a time without breaking the ones that aren't ready.
    """
    if hasattr(model, "predict_extra_fairness"):
        try:
            extra = model.predict_extra_fairness(**kwargs)
            result = {**result, **extra}
        except Exception as e:
            print(f"[WARN] predict_extra_fairness failed for "
                  f"{type(model).__name__}: {e}")
    return result
...
def save_summary(all_results: list, args: argparse.Namespace):
    ...
    row = {
        ...
        "runs":             args.runs,
        "seed":         args.seed,
        "split_seed":   getattr(args, "split_seed", 20),
        "lr":           args.lr,
    ...
    dedup_keys = ["dataset", "task", "model", "split_seed"]
...
def run_model(args, param1, param2, seed: int) -> dict:
    ...
    if model_name == "CrossWalk":
        ...
        adj, feats, labels, idx_train, idx_val, idx_test, sens, sens_idx = load_data(
            dataset, feature_normalize=_fn(dataset in ("nba", "german"), args),
            split_seed=getattr(args, "split_seed", 20))
        graph_stats = get_graph_stats_adj(adj)
        model = _model_class("CrossWalk")()
        t_fit = time.time()
        ...
        result = {**pack_result(model.predict(idx_test)),
                  **pack_scale(graph_stats, model, epochs_run, fit_sec)}
        return _maybe_add_extra_fairness(model, result, idx_test=idx_test)

    elif model_name == "FairWalk":
        # walk 기반 — 내부 embedding lr 고정, 외부 설정 미지원
        adj, feats, labels, idx_train, idx_val, idx_test, sens, sens_idx = load_data(
            dataset, split_seed=getattr(args, "split_seed", 20))
        graph_stats = get_graph_stats_adj(adj)
        model = _model_class("FairWalk")()
        t_fit = time.time()
        model.fit(adj, labels, idx_train, sens, device=device,
                  num_walks=param1, walk_length=param2)
        fit_sec = time.time() - t_fit
        epochs_run = _infer_epochs_run(model, fallback=int(param1))
        result = {**pack_result(model.predict(idx_test, idx_val)),
                  **pack_scale(graph_stats, model, epochs_run, fit_sec)}
        return _maybe_add_extra_fairness(model, result, idx_test=idx_test)
        ...
        adj, feats, labels, idx_train, idx_val, idx_test, sens, sens_idx = load_data(
            dataset, feature_normalize=_fn((dataset == "german"), args),
            split_seed=getattr(args, "split_seed", 20))
        graph_stats = get_graph_stats_adj(adj)
        model = _model_class("FairVGNN")()
        t_fit = time.time()
        ...
        model = _model_class("FairGNN")(feats.shape[-1], acc=temp_acc,
                        epoch=epochs,
                        # alpha weights the covariance constraint and beta the
                        # adversarial loss, separately, in
                        # G_loss = cls + alpha*cov - beta*adv (FairGNN.py:131)
                        alpha=(0.0 if _fgnn_off("cov", args) else param1),
                        beta=(0.0 if _fgnn_off("adv", args) else param2),
                        ).to(device)
        ...
        adj, feats, labels, idx_train, idx_val, idx_test, sens, sens_idx = load_data(
            dataset, feature_normalize=_fn(False, args),
            split_seed=getattr(args, "split_seed", 20))
        graph_stats = get_graph_stats_adj(adj)
        if dataset in ("credit", "german"):
            feats = feats / feats.norm(dim=0)
        ep = 100 if dataset == "german" else min(500, epochs)
        model = _model_class("EDITS")(feats, dropout=param1, lr=lr, weight_decay=wd)
        t_fit = time.time()
        ...
        result = {**result, **pack_scale(graph_stats, model, epochs_run, fit_sec)}
        return _maybe_add_extra_fairness(model, result, idx_test=idx_test)
        ...
        adj, feats, labels, idx_train, idx_val, idx_test, sens, sens_idx = load_data(
            ...
            split_seed=getattr(args, "split_seed", 20))
        graph_stats = get_graph_stats_adj(adj)
        # sim_coeff weights NIFTY's counterfactual//stability terms; 0 leaves the
        # same encoder, protocol and selection with no fairness objective, which
        # is what "this method's own backbone" means for NIFTY. Unset, the
        # algorithm's own default (0.5) applies and the branch is unchanged.
        _off = set(getattr(args, "components_off", []) or [])
        if getattr(args, "fairness_off", False):
            _off |= {"obj", "lip"}          # legacy flag == every component off
        _nifty_kw = {}
        if "obj" in _off:
            _nifty_kw["sim_coeff"] = 0.0
        if "lip" in _off:
            _nifty_kw["use_spectral_norm"] = False
        model = _model_class("NIFTY")(adj, feats, labels, idx_train, idx_val, idx_test, sens, sens_idx,
                      num_hidden=args.hidden_dim,
                      num_proj_hidden=args.proj_hidden_dim,
                      lr=lr, weight_decay=wd, device=device, **_nifty_kw)
        t_fit = time.time()
        ...
        adj, feats, labels, idx_train, idx_val, idx_test, sens, sens_idx = load_data(
            ...
            split_seed=getattr(args, "split_seed", 20))
        graph_stats = get_graph_stats_adj(adj)
        model = _model_class("GNN")(adj, feats, labels, idx_train, idx_val, idx_test, sens, sens_idx,
                    num_hidden=args.hidden_dim,
                    num_proj_hidden=args.proj_hidden_dim,
                    encoder=getattr(args, "gnn_encoder", "gcn"),
        ...
        data, sens_idx, x_min, x_max = get_dataset(dataset, split_seed=getattr(args, "split_seed", 20))
        graph_stats = get_graph_stats_pyg(data)
        data.sens_idx = sens_idx
        model = _model_class("FairGB")()
        t_fit = time.time()
        ...
        result = {**pack_result(model.predict()),
                  **pack_scale(graph_stats, model, epochs_run, fit_sec)}
        # NOTE: this branch uses get_dataset() (PyG-style `data`), not
        # load_data() -- there is no `idx_test` variable here, unlike the
        # walk-based / FairGNN branches above. Derive it from data.test_mask.
        fairgb_idx_test = data.test_mask.nonzero(as_tuple=False).view(-1)
        return _maybe_add_extra_fairness(model, result, idx_test=fairgb_idx_test)

    elif model_name == "FairGT":
        data, sens_idx, _, _ = get_dataset(dataset, split_seed=getattr(args, "split_seed", 20))
        graph_stats = get_graph_stats_pyg(data)
        model = _model_class("FairGT")(data, sens_idx, args, lr=lr, weight_decay=wd, device=device)
        t_fit = time.time()
        ...
        result = {**pack_result(model.predict()),
                  **pack_scale(graph_stats, model, epochs_run, fit_sec)}
        fairgt_idx_test = data.test_mask.nonzero(as_tuple=False).view(-1)
        return _maybe_add_extra_fairness(model, result, idx_test=fairgt_idx_test)
...
def parse_args():
    ...
    g.add_argument("--split_seed",   type=int,   default=20,
                   help="jMfU Q4: train/val/test split RNG seed (utils/data.py, "
                        "honoured by every model: load_data() now forwards it to "
                        "get_dataset(). Default 20 reproduces Table 1 splits.")
    g.add_argument("--gnn_encoder", type=str, default="gcn",
                   choices=["gcn", "sage", "gin", "jk", "infomax"],
                   help="encoder for the plain GNN baseline. FairGB runs on "
                        "SAGE by its own default, so comparing it against a GCN "
                        "baseline credits the architecture to the method.")
    g.add_argument("--components_off", nargs="*", default=[],
                   help="switch off named fairness components, leaving "
                        "everything else identical. Each method names its own, "
                        "taken from what its paper claims:\n"
                        "  FairGNN  cov  (alpha, covariance constraint)\n"
                        "           adv  (beta, adversarial loss)\n"
                        "  NIFTY    obj  (sim_coeff, counterfactual objective)\n"
                        "           lip  (spectral_norm, Lipschitz constraint)\n"
                        "  FairGB   cal  (contribution alignment)\n"
                        "           cnm  (counterfactual node mixup)\n"
                        "  FairGT   eig  (eigenvector positional encoding)\n"
                        "           sgr  (same-sensitive complete graph)\n"
                        "           khp  (k-hop feature encoding)\n"
                        "  BeMap    bal  (balanced neighbour sampling)\n"
                        "Empty (the default) changes nothing. All of a method's "
                        "components off is its M0.")
    g.add_argument("--fairness_off", action="store_true",
                   help="disable the method's fairness mechanism where it is a "
                        "coefficient the method already exposes, leaving the "
                        "same backbone and protocol. FairGNN: alpha = beta = 0. "
                        "NIFTY: sim_coeff = 0. Nothing else is touched; methods "
                        "whose mechanism is not a coefficient are not supported "
                        "and must be compared against a matched plain encoder "
                        "instead.")
    g.add_argument("--force_feature_normalize", type=str, default=None,
                   choices=["0", "1"],
                   help="override every branch's feature_normalize. "
                        "Unset reproduces the submitted comparison.")
