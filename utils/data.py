...
def load_income(dataset="income", path="./data/income/", label_number=6000, feature_normalize=True,
    ...
    idx_train, idx_val, idx_test = balanced_split(labels, label_number, seed=split_seed)
...
def load_german(dataset="german", path="./data/german/", label_number=100, feature_normalize=True,
    ...
    # pandas 3 refuses to write an int into a str column, which is what the two
    # original .loc assignments did; under pandas 2 they silently upcast the
    # column to object and every downstream use went through
    # sp.csr_matrix(..., dtype=float32), so the values seen by the model were
    # 1.0 / 0.0. Mapping to int reproduces exactly that, and raises rather than
    # producing NaN if the column ever holds something other than the two
    # labels the original code assumed.
    _g = df["Gender"].map({"Female": 1, "Male": 0})
    if _g.isna().any():
        raise ValueError(
            f"unexpected Gender values: {sorted(set(df['Gender'][_g.isna()]))}")
    df["Gender"] = _g.astype(int)
    ...
    features = torch.FloatTensor(np.array(sp.csr_matrix(df[header], dtype=np.float32).todense()))
    labels_np = df[predict_attr].values.copy()   # pandas 3 returns a
    # read-only view; the next line writes into it in place
...
def get_dataset(dataname: str, path: str = "./data", feature_normalize: bool = True,
                 split_seed: int = 20) -> Tuple[Data, int, torch.Tensor, torch.Tensor]:
    """
    ...
    split_seed: jMfU Q4 rebuttal. Controls the train/val/test split RNG
        (previously hardcoded to 20 everywhere in this file, with no way
        to vary it from the caller). Default (20) exactly reproduces every
        split used to generate the paper's Table 1 results.
    """
    if dataname == "credit":
        return load_credit(
            dataset="credit",
