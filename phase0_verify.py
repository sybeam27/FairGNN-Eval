...
# schema change: per_unit_metrics now carries method + configuration separately
COORDS = [("dAUC", "auc", 1), ("negDP", "dp", -1), ("negEO", "eo", -1)]
rng = np.random.default_rng(0)


def unit_contrasts(method, dataset, configuration=None, protocol="controlled", selector="common_bce"):
    s = u[(u.method == method) & (u.dataset == dataset)
          & (u.protocol == protocol) & (u.selector == selector)]
    if configuration is not None:
        s = s[s.configuration == configuration]
...
for _, r in m.iterrows():
    C = unit_contrasts(r.method, r.dataset, r.configuration)
    for c, _, _ in COORDS:
        x = C[("I", c)]
        mu = x.mean()
        est = hier_boot(x)
